...
// Данные сайта грузятся из GitHub и живут в памяти, пока сервер работает.
...
// На всякий случай убеждаемся, что данные точно загружены, прежде чем
// обрабатывать запрос (важно для serverless-платформ вроде Vercel —
// подробности в комментарии к store.ensureLoaded()).
app.use((req, res, next) => {
  store.ensureLoaded().then(() => next(), next);
});
...
// Сервер начинает слушать порт сразу — не дожидаясь загрузки данных
// с GitHub. Само по себе это безопасно: каждый запрос всё равно ждёт
// готовности данных через middleware ensureLoaded() выше. Загрузку
// запускаем сразу же, не дожидаясь первого запроса, чтобы она шла
// параллельно, а не только по факту первого обращения.
...
// Явный экспорт приложения — так Vercel гарантированно распознаёт его как
// serverless-функцию. На Render и при локальном запуске (node server.js)
// эта строка ни на что не влияет, сайт запускается как обычно через
// app.listen() выше.
module.exports = app;
